%% This script is used to compare the performance of global optimization algorithms 
% on large-scale test problems
% Author: Bin She
...
dimensions = [200, 300, 400, 500, 600, 700, 800, 900, 1000];
...
benchmarks = {    
    ...
    @bsSchwefel2_22, "Schwefel's P2.22", @(dim)(zeros(dim, 1)), @(dim)(0), [-5 5], dimensions;
    @bsRosenbrock, 'Rosenbrock', @(dim)(ones(dim, 1)), @(dim)(0), [-100 100], dimensions;
    {@bsCFSingle, @(dim)(abs(rand(3, 1)+0.1))}, 'CF1', @(dim)(ones(dim, 1)), @(dim)(0), [-5 5], dimensions;
    {@bsStochasticRosenbrock, @(dim)(abs(rand(dim, 1)) + 0.5)}, 'Stochastic Rosenbrock', @(dim)(ones(dim, 1)), @(dim)(0), [-100 100], dimensions;
    @bsXinSheYang, 'Xin-She Yang', @(dim)(zeros(dim, 1)), @(dim)(0), [-2 2], dimensions;
    ...
    {@bsShiftedRastrigin, @(dim)(rand(dim, 1)-0.5)*50}, 'Shifted Rastrigin', @(dim)(zeros(dim, 1)), @(dim)(0), [-100 100], dimensions;
    @bsSchwefel, 'Schwefel', @(dim)(420.9687*ones(dim, 1)), @(dim)(0), [-500 500], dimensions;
    {@bsCFComplex, @(dim)([abs(rand(5, 1)+0.1)])}, 'CF2', @(dim)(ones(dim, 1)), @(dim)(0), [-2 2], dimensions;
};
...
%     'PSO', 'MATLAB', {NInitFcn};     % 100 refers to the initial population size of the PSO 
%     'GA', 'MATLAB', {NInitFcn};      % 100 refers to the initial population size of the PSO 
    % the special parameters of the following 2 methods are 
    % NInit
    % p_a (switching parameters)
    % alpha
    % population initial function
%     'CS', 'Yang and Deb', {NInitFcn 0.75 0.05, @bsGenerateInitialPopulationByRandom}; 
%     'GBCS', 'Fateen', {NInitFcn, 0.75, 0.05, @bsGenerateInitialPopulationByRandom};
...
%     'AGBCS', 'She et al.', {NInitFcn, 0.75, 0.05, @bsGenerateInitialPopulationByRandom, 50, 200};
%     'AHSACS', 'She et al.', {NInitFcn, 5, @(nDim)(2*nDim), @bsGenerateInitialPopulationByRandom};
    'HAGCS', 'She et al.', {NInitFcn, 5, @(nDim)(2*nDim), @bsGenerateInitialPopulationByRandom, 50, 200};
};
...
for iBenchmark = 1 : nBenchmarks
    ...
    save Table_Large_Scale_Results;
end
